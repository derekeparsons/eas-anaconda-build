...
INST_DIR="/opt/anaconda2"
FLAGS="-b -f -p $INST_DIR"
OS="$(/usr/bin/uname)"
USR="$SUDO_USER"
OSX_URL="$(wget -O - https://www.anaconda.com/distribution/ 2>/dev/null | sed -ne 's@.*\(https:\/\/repo\.anaconda\.com\/archive\/Anaconda2-.*-MacOSX-x86_64\.sh\)\">64-Bit Command Line Installer.*@\1@p')"
LINUX_URL="$(wget -O - https://www.anaconda.com/distribution/ 2>/dev/null | sed -ne 's@.*\(https:\/\/repo\.anaconda\.com\/archive\/Anaconda2-.*-Linux-x86_64\.sh\)\">64-Bit (x86) Installer.*@\1@p')"
...
then
	...
	then
		...
		OSX_INST="$(ls -ct | egrep 'Anaconda2-.*-MacOSX' | head -1)"
		if [ ! -z "$OSX_INST" ]
		then
...
#sudo -i -u $USR bash $INST_DIR/bin/conda update anaconda --yes
